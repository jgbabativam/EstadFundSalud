...
$$F = \frac{S_1^2}{S_2^2} \quad \text{(bajo } H_0: \sigma_1^2=\sigma_2^2\text{)}$$
...
Un **contraste de hipótesis** es una regla de decisión estadística que permite, a partir de los datos muestrales, pronunciarse sobre una afirmación sobre un parámetro poblacional.
...
annotate( "text", x = critico_der + 0.25, y = dnorm(critico_der, mu0, se)/2, label = expression(alpha), color = "#d62828", size = 7, fontface = "bold" ) + 
  annotate( "text", x = critico_izq - 0.25, y = dnorm(critico_izq, mu0, se)/2, label = expression(alpha), color = "#d62828", size = 7, fontface = "bold" ) +
...
$gl = (2-1)(2-1) = 1$; $\chi^2_{0.05,1} = 3.841$
...
## Visualización: mosaico y barras {style="font-size: 85%;"}
...
## Código de R {style="font-size: 88%;"}
...
$$(\hat{p}_1 - \hat{p}_2) \pm z_{\alpha/2} \sqrt{\hat{p}(1-\hat{p})\left(\frac{1}{n_1}+\frac{1}{n_2}\right)}$$
...
$$Z = \frac{0.225-0.107}{\sqrt{0.168 \times 0.832 \times (1/80+1/75)}} = 2.26$$

$p\text{-valor} = 2 \times P(Z > 2.26) = 0.024 < 0.05$

**→ Se rechaza $H_0$. Diferencia significativa.**


## Código R: dos proporciones
...
::: {.callout-tip}
El IC 95% para $p_A - p_B$ es $[0.015,\; 0.220]$. Como no contiene el 0, confirma diferencia significativa. La UCI A tiene entre 1.5 y 22 puntos porcentuales más de infecciones que la UCI B.
:::
...
**Ejemplo:** El umbral de buen control glucémico en diabetes tipo 2 es $HbA1c = 7\%$. En 20 pacientes de un programa de rehabilitación se midió: $\bar{x} = 7.8\%$, $s = 1.2\%$.

$H_0: \mu = 7$ vs $H_a: \mu \neq 7$

$$t = \frac{7.8 - 7}{1.2/\sqrt{20}} = \frac{0.8}{0.268} = 2.98$$

$gl = 19$; $t_{0.025, 19} = 2.093$

Como $|t| = 2.98 > 2.093$ **→ Rechazar $H_0$**.

$p = 0.008 < 0.05$. El nivel medio de HbA1c es significativamente mayor que 7%.
...
ggplot(df_hba, aes(x=x)) +
  ...
  geom_density(color="#e63946", linewidth=1.3) +
  geom_vline(xintercept=c(mean(HbA1c), 7),
             color=c("#e63946","#212121"), linewidth=1.3,
             linetype=c("solid","dashed")) +
  annotate("text", x=mean(HbA1c)+0.08, y=0.55, label=paste0("x̄=",round(mean(HbA1c),2)),
           color="#e63946", size=4.5) +
  annotate("text", x=7.08, y=0.55, label="μ₀=7", color="#212121", size=4.5) +
...
$$\boxed{t = \frac{\bar{x}_1 - \bar{x}_2}{s_p\sqrt{1/n_1+1/n_2}}}$$
...
**Ejemplo:** Tiempo de hospitalización (días) en dos grupos de pacientes con neumonía: Grupo A (tratamiento estándar, $n_1=12$): $\bar{x}_1=6.8$, $s_1=1.5$; Grupo B (protocolo nuevo, $n_2=12$): $\bar{x}_2=5.3$, $s_2=1.4$.

$s_p^2 = \frac{11 \times 1.5^2 + 11 \times 1.4^2}{22} = \frac{24.75+21.56}{22} = 2.105$

$$t = \frac{6.8-5.3}{\sqrt{2.105(1/12+1/12)}} = \frac{1.5}{0.593} = 2.53$$

$gl=22$; $t_{0.025,22}=2.074$. **→ Rechazar $H_0$** ($p=0.019$).
...
Compara medias de dos mediciones en los mismos sujetos (antes-después, lado derecho-izquierdo, etc.).
...
$$\boxed{t = \frac{\bar{d}}{s_d/\sqrt{n}}} \sim t(n-1) \text{ bajo } H_0$$
...
$t = \frac{21.5}{3.34/\sqrt{8}} = \frac{21.5}{1.181} = 18.2$
...
$$\boxed{F = \frac{S_1^2}{S_2^2}} \sim F(n_1-1,\; n_2-1) \text{ bajo } H_0$$
